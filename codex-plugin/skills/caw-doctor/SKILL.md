...
ルート `office/AGENTS.md`（Claude Code では `CLAUDE.md`）を読み、冒頭の **`> トラック: 就活`** の有無でトラックを判定する。
...
- [ ] `office/` が存在する
- [ ] `office/AGENTS.md` が存在し、空でない
- [ ] `office/secretary/` が存在する
- [ ] `office/secretary/AGENTS.md` が存在する
...
```bash
# office/<dept>/ の一覧と各 AGENTS.md の有無
