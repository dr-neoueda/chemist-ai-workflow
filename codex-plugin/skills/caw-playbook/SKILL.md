...
- ユーザーが「◯◯の Playbook を web から種まきして」と言ったとき → 初期リファレンスの単発取得。**そのツール 1 つだけ** caw スキルの `references/playbook-web-seeding.md` の手順で `## 外部リファレンス（web 由来・要検証）` に出典 URL つきで既定・罠を追記する（`## Lessons Learned` は本人の実測に予約）。**常に 1 ツール単位・オンボーディングでは種まきしない**
- 計算ジョブ完了後の振り返り段階
...
- **それ以外（Codex / Gemini、または memory 機能を使っていない）**：`office/computation/AGENTS.md`（Claude Code は `CLAUDE.md`、Gemini は `GEMINI.md`）の「共通知見」節、または秘書の `secretary/notes/` に 1 行で記録。
