...
- **遅延方式（既定）**：各ツールの初期 Playbook 種まきは、**そのツールを初めて使うとき**に `caw-input` がオファーする（対象 Playbook が cold-start＝`## Lessons Learned` も `## 外部リファレンス` も空なら、そのツール 1 つだけ種まきするか 1 回尋ねる）。手順・書式・規律は `references/playbook-web-seeding.md`。
- **明示要求**：「◯◯の Playbook を web から種まきして」と言われたら、そのツール 1 つに同手順を単発実行する。
