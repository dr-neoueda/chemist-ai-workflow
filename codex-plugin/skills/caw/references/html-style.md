...
## 横棒チャート（数値データの定番・そのままコピーして使う）

セグメント売上・利益、シェア、ランキングなど **量の比較は表だけにせず横棒で可視化**する。1 系列 = 1 行の `<svg>` を縦に並べる。主役（自社・対象）は `--accent`（#aa2d00）、比較対象は `#888888`。

```html
<svg viewBox="0 0 470 26" width="470" height="26" role="img" aria-label="電子材料 10157億円">
  <text x="0" y="18" font-size="12.5" fill="#333840">電子材料</text>
  <rect x="120" y="7" width="280" height="12" fill="#eeeeee" rx="6"/>   <!-- グレーのトラック（最大幅 280） -->
  <rect x="120" y="7" width="280" height="12" fill="#aa2d00" rx="6"/>   <!-- 値バー: width = 280 × 値 ÷ 系列内最大値 -->
  <text x="406" y="18" font-size="12" fill="#555555">10,157</text>      <!-- 値ラベル: x = 値バー右端 + 6 -->
</svg>
```

- 4 要素＝**ラベル左 → グレーのトラック → 色付きの値バー → 数値**。値バー幅は `280 × その値 ÷ 系列内最大値`。
- レーダー（多軸比較）・散布（2 軸ポジショニング）も同じ 2 色方針。見出し id と `<svg>` の id は別名（`ch` 接頭辞）。
