...
未収載（フルポート時に追加予定）: `caw-slides` / `caw-register` / `caw-write` / `caw-input` / `caw-playbook` / `caw-doctor`、`hooks.json`、`.mcp.json`。
...
- 同梱スキルは `caw` / `caw-setup` の 2 つ（PoC）。残りの 6 スキル（caw-doctor / caw-input / caw-register / caw-write /
