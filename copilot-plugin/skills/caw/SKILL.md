...
**重要**：`office/.mcp-setup.md` は **手順書**であり、API key そのものは絶対に書かない（環境変数経由で渡す手順のみ記載）。GitHub Copilot CLI では MCP サーバを `.mcp.json`（標準 `mcpServers` 形式。プラグイン同梱の `.mcp.json`、またはワークスペース／ユーザの MCP 設定）で設定する。Quick モードでは Q3/Q4 未取得のため、MCP セットアップは生成せず「caw で後から生成できます」と案内するに留める。
