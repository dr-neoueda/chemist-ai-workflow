...
- **はじめて** → Call 1Q のみ実施（最小） + **はじめてモードを有効化**（`office/AGENTS.md` に記録）→ Step 3（**全 8 部署を scaffold**）→ START HERE 文書生成 → **初回ツアー**（Step 3-8）
...
Q2〜Q4 は「未定」扱い（後から caw で補完）。**部署はモード共通で全 8 部署を作成する**。`office/AGENTS.md` には研究分野のみ反映し、他は `{{未設定}}` プレースホルダで「caw で後から設定を足せます」と注記。
...
**詳しく**モードでのみ実施。回答は各部署 AGENTS.md のパーソナライズメモに反映し、運用初期から精度を上げる。
...
**詳しく**で得た回答は `office/AGENTS.md` の「パーソナライズメモ」に箇条書きで保存し、各部署が文脈として参照できるようにする。
...
- **はじめて** → `office/` + ルート AGENTS.md + **全 8 部署**（secretary / research / engineering / computation / analysis / writing / review / presentation）+ 作業ディレクトリ。Q2〜Q4 は未取得なので該当プレースホルダは `{{未設定}}`。完了メッセージで「caw で設定を後から足せます」と案内
- **通常** → `office/` + ルート AGENTS.md + **全 8 部署** + 作業ディレクトリ（Q1〜Q4 を各部署 AGENTS.md に反映）
- **詳しく** → 通常と同じ + Call 2（Q6〜Q9）の回答を `office/AGENTS.md` のパーソナライズメモと各部署 AGENTS.md に反映 + Q8 で申請書予定があれば writing 部署に申請書トラッカー雛形を追加
...
**重要**：`office/.mcp-setup.md` は **手順書**であり、API key そのものは絶対に書かない（環境変数経由で渡す手順のみ記載）。GitHub Copilot CLI では MCP サーバを `.mcp.json`（標準 `mcpServers` 形式。プラグイン同梱の `.mcp.json`、またはワークスペース／ユーザの MCP 設定）で設定する。はじめてモードでは Q3/Q4 未取得のため、MCP セットアップは生成せず「caw で後から生成できます」と案内するに留める。
...
2. **まず何をするか** — 3 ステップ（① `copilot`（GitHub Copilot CLI）を起動 → ② 「今日やることを教えて」等と話しかける → ③ 困ったら「ヘルプ」「〇〇って何?」と言う）
