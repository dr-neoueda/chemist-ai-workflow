...
- 名指しされたツール**1 つにつき 1 エージェント**（並列サブエージェントが使えるなら並列、無ければ**順次**）で処理する。各エージェントは「対象ツール名＋ユーザーの研究分野（Call 1〜2）」を受け、**web 検索で一次資料・公式ドキュメント**を調べ、`office/computation/playbooks/<tool>.md` の **`## 外部リファレンス（web 由来・要検証）`** セクションに追記する（`## Lessons Learned` には触れない）。
- **計算入力の構文が絡む種**は、書き込み前にクロスチェックする。
- **web 検索が使えない場合はスキップ**し、「後から Playbook に追記できます」と案内する。全エージェント完了後に **1 回だけ**「N ツールの初期 Playbook を各 `## 外部リファレンス（web 由来・要検証）` に置きました（web 由来・要検証。実際に回した知見は Lessons Learned に書けば上書きされます）」と通知する。
