...
4. **スキルの 3 パターン**：
   - 収集 → 構造化 → HTML 可視化（例: caw-paper / caw-company / caw-events）
...
| caw-paper（論文 → 構造化 → 登録） | caw-company / caw-events（企業・イベント → 構造化 → HTML） | 収集 → 構造化 → 可視化 | 公開情報の構造化・出典方針・HTML 可視化が機能する |
...
- 新スキルは §0 の **3 パターン（収集→構造化→可視化／生成→制約チェック／反復→ログ）** のどれかに沿わせる
