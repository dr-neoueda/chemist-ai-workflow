...
### Call 1J: 就活プロファイル（QJ1〜QJ3・全モード共通で聞く）
...
- **QJ1〜QJ3 は全モード共通で聞く**。区分・志望業界・悩みは「プログラミング / AI が初心者か」とは無関係なので、**はじめてモードでも省略しない**。経験レベル（Call 0）は説明の丁寧さ・初回ツアーの有無を変えるだけで、就活プロファイルの質問数は減らさない。
...
はじめてモードなら続けて `> 運用モード: はじめて（強めに誘導）` も書く。
