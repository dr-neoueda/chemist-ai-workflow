...
> **GitHub Copilot CLI での設定方法**：本テンプレート中のコマンド例は Claude Code 形式（`claude mcp add ...`）で記載しています。GitHub Copilot CLI では同じ MCP サーバを **`.mcp.json`（標準 `mcpServers` 形式）**で設定します（プラグイン同梱の `.mcp.json`、またはワークスペース／ユーザの MCP 設定）。env / 認証情報の考え方は CLI 共通です。具体形は末尾「GitHub Copilot CLI での設定」を参照。
...
- research 部署が `work/papers/md/<author-year>.md` を生成 → Notion Paper DB に同期
...
- research 部署が生成した `work/papers/md/<author-year>.md` を vault にコピー（frontmatter 付き）
...
- research 部署が `work/papers/pdf/` の PDF を Drive にアップロード → 共有 URL を md の `pdf_url` に書き戻し
...
## GitHub Copilot CLI での設定（`.mcp.json`）

GitHub Copilot CLI は MCP サーバを **JSON 設定（`.mcp.json`、標準 `mcpServers` 形式）**で読み込む。プラグイン同梱の `.mcp.json`、またはワークスペース／ユーザの MCP 設定に次の形で記述する（上の各 `claude mcp add ...` 例と等価）：
...
**環境変数（`NOTION_API_KEY` 等）を `~/.zshrc` / `~/.bashrc`（Windows は環境変数）に置けば、Copilot・Claude・Codex のどの CLI からも同じ鍵を参照でき、二重管理を避けられる。**
