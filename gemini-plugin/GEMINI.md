...
### 研究オンボーディング
1. **経験レベル**（はじめて / 通常 / 詳しく）。どのモードでも作成部署は同じ（全 8 部署）。変わるのは質問の深さだけ。
2. **研究プロファイル**：研究分野（有機/物理/材料/計算 等）／計算ソフトのカテゴリ（量子化学・古典 MD・周期系 DFT・使わない、複数可）／ナレッジベース（Notion/Obsidian/Logseq）／クラウドストレージ（Google Drive/Dropbox/OneDrive）。「詳しく」なら HPC・共著・申請書・論文ステータスも。
3. **scaffold**（下記）→ `office/GEMINI.md` 生成（冒頭に `> トラック: 研究`、はじめてなら `> 運用モード: はじめて`）→ START HERE 文書 → 「何をしますか？」
...
- **部署はモードに応じて全部作成**（ユーザーに「どの部署か」を尋ねない）。
