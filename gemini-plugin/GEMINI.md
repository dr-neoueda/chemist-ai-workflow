...
**手順**：(1) SVG を上記規約で描く（1 スライド 1 svg・作業層 `_src/<deck>/`。論文図は高解像度切り抜きして data-URI 埋込＋source caption。**ユーザーが `work/presentations/figures/` に置いた画像**（顕微鏡写真・装置スクショ・外部プロット等）も候補として拾い、**アスペクト比を保って** data-URI 埋込（変換前にローカル image href を data-URI 化）。自作図表は native shape。**数式は LaTeX を透過 PNG にレンダして data-URI 埋込**＝matplotlib mathtext のオフライン既定、`\ce{}` 等フル LaTeX のみオンライン codecogs）(2) 目視で豆腐・重なり・はみ出しを潰す (3) `svg_to_pptx` 変換器で native pptx 化＋CJK run の ea を MS Gothic に後処理 (4) 全ページ目視。**変換器は Gemini 版に同梱しないため、PPT Master（`hugohe3/ppt-master`, MIT）か caw plugin の `vendor/svg_to_pptx` を用意する**（依存: python-pptx、画像時 Pillow、切り抜き時 PyMuPDF、数式時 matplotlib）。**配布先 `work/presentations/slides/` には pptx 1 ファイルのみ**（preview PNG・中間 SVG は置かない）。
