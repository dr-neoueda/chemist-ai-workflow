...
name: caw-doctor
description: >
  `.company/` 構造の健全性チェックと修復提案。ルート CLAUDE.md・秘書部・各部署 CLAUDE.md の存在確認、
  旧構造（成果物が `.company/<dept>/X/` に入っている）の検出、Playbook 更新滞り、
  同日ファイル重複などを総点検し、修復コマンドを提示する。
trigger: /caw-doctor
---
...
- [ ] `.company/` が存在する
- [ ] `.company/CLAUDE.md` が存在し、空でない
- [ ] `.company/secretary/` が存在する
- [ ] `.company/secretary/CLAUDE.md` が存在する
...
```bash
# .company/<dept>/ の一覧と各 CLAUDE.md の有無
...
選択された部署を `.company/CLAUDE.md` の「部署一覧」テーブルから読み取り、対応する top-level dir の存在をチェック。
