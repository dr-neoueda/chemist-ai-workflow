...
`office/`（就活モード）が無ければ `/caw` で就活トラックのセットアップを促す。 **また、office 冒頭が `> トラック: 研究` の場合は「これは就活向けのスキルです。研究プロジェクトのようなので caw-research（論文検索）や caw-write 等をご利用ください」と伝え、無理に進めない。**書類部が未作成なら作成を提案する。
...
- **`work/documents/writing-preferences.md`（推敲で学んだ好み）があれば必ず読み、文体プロファイルの上に重ねて先回り適用する**（例:「結論を強く」「冗長表現を避ける」「具体的な数字を必ず入れる」等、前回までの直しを次の ES に持ち越す）。
...
**好みの学習（毎回・自分から確認）**：**推敲で修正を受けるたびに必ず**、その直しが汎用的な好み（今後の ES でも効く語調・構成・避ける表現など）か、その設問固有の一度きりの直しかを評価する。汎用的なら**ユーザーに言われるのを待たず自分から**「次回以降もこの直し方を既定にしますか？」と確認し、OK なら `work/documents/writing-preferences.md` に 1 行ルールで追記する。**この挙動は Claude Code / Codex CLI / Gemini CLI で共通**。**一度きりの言い回し直しは記録しない**（聞きすぎない）。
...
### 推敲からの好みの学習（`work/documents/writing-preferences.md`）

文体プロファイル（過去文書からの初期文体）とは別に、**推敲で繰り返し出る好み**（語調・構成・避ける表現）を、確認のうえ 1 行ルールで `work/documents/writing-preferences.md` に蓄積し、次の ES から先回り適用する。汎用的な好みのみ記録し、一度きりの直しは記録しない。誤ルールは全 ES を劣化させるため、追記前に必ず確認する。`office/` 設定は書き換えない（計算側 caw-input ↔ caw-playbook 学習ループの作文版）。
...
- **推敲で汎用的な直しを受けたら、言われる前に自分から確認のうえ `writing-preferences.md` に蓄積し、次の ES に持ち越す**（毎回評価する・全 CLI 共通・一度きりの直しは記録しない）
