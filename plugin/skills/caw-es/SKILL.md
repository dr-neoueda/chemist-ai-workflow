...
**好みの学習（確認してから記録）**：修正が**その設問固有でなく汎用的な好み**（今後の ES でも効く語調・構成・避ける表現など）だと判断したら、「次回以降もこの直し方を既定にしますか？」と確認し、OK なら `work/documents/writing-preferences.md` に 1 行ルールで追記する。**一度きりの言い回し直しは記録しない**。
...
- **推敲の汎用的な直しは確認のうえ `writing-preferences.md` に蓄積し、次の ES に持ち越す**（一度きりの直しは記録しない）
