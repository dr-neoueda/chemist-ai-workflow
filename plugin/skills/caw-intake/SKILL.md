...
このスキルを実行する前に `office/CLAUDE.md`（Codex CLI / GitHub Copilot CLI では `AGENTS.md`）を読み、冒頭に `> 運用モード: はじめて` があれば、`caw` skill の「はじめてモードの挙動」を全応答に適用する：**平易な日本語**で話し、専門用語（voice プロファイル・STAR・汎関数 等）は初出で 1 行説明を添え、各ステップの最後に**「次はこれをしましょう」を 1 つ**だけ提示する。
...
`office/CLAUDE.md`（`AGENTS.md`）の冒頭を読み、`> トラック:` が `就活` なら**就活トラック**、`研究` なら**研究トラック**。**行が無い（旧 office）場合は `work/companies/` があれば就活、`work/papers/`・`work/topics/` があれば研究と推定し、判別できなければ 1 問尋ねる**。判定後、下記の分類表を使う。
