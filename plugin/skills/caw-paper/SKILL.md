...
- 既存の `.company/research/papers/` に書誌情報を追加したいとき

`.company/research/` が存在しない場合、ユーザーに `/caw` で research 部署を追加することを促す。
...
`.company/research/topics/<topic-slug>.md` を生成。フォーマット：
...
- `.company/CLAUDE.md` の「オーナープロフィール」で **ナレッジベース** と **クラウドストレージ** が指定されている
...
新規追加された PDF を検出（既に `.company/research/papers/` に登録済の md と照合してスキップ）。
...
`.company/research/papers/<author-year-keyword>.md` に書誌情報付き md を生成。命名規則：
...
- データソース ID をユーザーから取得（`.company/CLAUDE.md` に保存されていれば使用、無ければ問い合わせ）
...
- `.company/research/papers/` 配下のローカル md のみで完結
