...
trigger: /caw-register
---

# caw-register — 論文の登録・管理（ライブラリ）

## いつ使うか

- `/caw-register` を実行したとき
...
**caw-register を起動したら、登録を始める前に必ず `AskUserQuestion` で「抽出レベル」を尋ねる。** PDF やファイルだけ渡された場合でも、**自動でレベルを決めず必ず一度尋ねてから着手する**。AI の使用量（トークン）はこのレベルで大きく変わるため、ユーザーに選ばせる。
...
`/caw-register --batch` でユーザーが起動した場合：
