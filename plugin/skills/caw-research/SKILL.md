...
- **件数**（既定 50、最大 100 程度）
- **期間 / ジャーナル絞り込み**（任意）
- **登録済み論文の扱い**：既定で `work/papers/md/` に**登録済み（md がある）論文は一覧から除外**する（目的は「まだ持っていない論文を見つける」ため）。全件見たいときはユーザーが「登録済みも含めて」と指定。
...
HTML に書き出す前に、`work/papers/md/` の登録済み md と照合し、**既に登録済みの論文を一覧から除く**：

1. `work/papers/md/*.md` の frontmatter から `doi`（あれば arXiv ID）を集めて「登録済みセット」を作る。ファイル名（`<著者-年-keyword>.md`）も補助に使う。
...
4. `work/papers/md/` が空／無ければ除外なし。**ユーザーが「登録済みも含めて」と言った場合は除外しない**（全件を載せる）。
...
- **フッタ**：「読みたい論文は PDF を `work/papers/pdf/` に置いて『登録して』と言うと `caw-register` が要約・登録します」。
...
<footer>読みたい論文は PDF を <code>work/papers/pdf/</code> に置いて「登録して」と言うと caw-register が要約・登録します。</footer>
</body></html>
```
...
- 読みたい論文を選んでもらい、「**PDF を `work/papers/pdf/` に置いて『登録して』と言えば `caw-register` が書誌付き要約と KB 登録をします**」と案内する（**PDF 自動ダウンロードは無し**＝ユーザーが手動で取得）。
