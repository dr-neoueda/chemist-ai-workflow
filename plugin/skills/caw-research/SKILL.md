...
description: >
  ...
  L2 スタンダード / L3 ディープ）と出力形式（md のみ / html のみ / 両方、既定は md）を発動時に選ばせ、companies/<企業名>.md に
...
発動したら、まず次の 3 つを確認する（はじめてモードでは特に丁寧に）。
...
3. **出力形式（既定は md）**：
   - **md のみ（既定）** — `companies/<企業>.md`（業界研究は `companies/_industry/<業界>.md`）に構造化。
   - **html のみ** — ブラウザで開ける `companies/<企業>.html`。
   - **両方** — md ＋ html。

> **既定は md**。ユーザーが出力形式を指定しなければ md のみで進める（HTML は重いので必要なときだけ）。「HTML で可視化して」等と言われた場合は html を既定に切り替える。
...
出力形式が **md** または **両方**なら、`companies/<企業名>.md`（業界研究は `companies/_industry/<業界名>.md`）に上の 8 ブロックを調査レベルの深さで書く。
...
- データは `companies/*.md`（または収集した事実）のみ。HTML のために数値を**捏造しない**。値が無い属性は「—」やグレー表示にする
...
- **既定の成果物は md**（`companies/<企業名>.md`）。HTML は出力形式で html / 両方 を選んだときだけ作る
- **一次情報（公式採用ページ・IR）を優先**。非公式（年収・口コミ等）は複数ソースで裏取り・断定しない
- 古い情報に注意（募集要項・採用条件は年度で変わる）。**取得日と調査レベルを併記**する
- 企業の非公開情報・個人情報はローカルに留める
- 成果物は top-level `companies/`（業界研究は `companies/_industry/`）に置く
