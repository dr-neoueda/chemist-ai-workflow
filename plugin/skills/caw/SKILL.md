...
- **就活** を選んだら → **`references/job-hunting-departments.md` を読み、その §A〜§E に従って**就活モードのオンボーディング・scaffold・START HERE・運営モードを実行する（以降の研究向け Call 0〜3 は使わない）。**就活トラックでは経験レベル（Call 0「ターミナル/AI は初めてか」）は聞かず、常にはじめてモード（平易）で進める**（office 設定に `> 運用モード: はじめて` を必ず書く）。
- **研究プロジェクト** を選んだら → そのまま下記の研究向けフローへ。

**（以下は研究トラックのオンボーディング）オンボーディングは 3 段階モード**。まず Call 0 でモードを選んでもらい、選んだモードに応じて質問数を変える。

#### Call 0: セットアップモード選択（1 問）

最初の質問は **経験レベルを率直に尋ねる**ことから始める。モード名（通常／詳しく）が
分からない人を取りこぼさないため、先頭に「初めてですか？」を置き、「はい、初めて」を一番上に強調する。
**どのモードを選んでも作成される部署は同じ（化学者モードは全 8 部署）。モードで変わるのは personalization 質問の深さだけ。**

```
Q0 (モード): 「はじめに 1 つだけ。パソコンのターミナルや AI エージェントを使うのは初めてですか？」
  - はい、初めて（強めに誘導してほしい） — はじめてモードで進める
  - いいえ：標準的に研究プロファイルを整えて構築したい（推奨） — 通常
  - いいえ：HPC・共著者・申請書まで詳しく整えたい — 詳しく
  (multiSelect: false; Other で自由入力可)
```

選択肢のラベルが指す内部モードは「はじめて / 通常 / 詳しく」。以後の分岐はこのモード名で記述する。
**どのモードでも、作成する部署はモード共通（化学者モードは全 8 部署）で固定。Quick（秘書のみ）モードは廃止した。**

- **はじめて** → Call 1Q のみ実施（最小） + **はじめてモードを有効化**（`office/CLAUDE.md` に記録）→ Step 3（**全 8 部署を scaffold**）→ START HERE 文書生成 → **初回ツアー**（Step 3-7）
- **通常** → Call 1 を実施 → Step 3（**全 8 部署を scaffold**）
- **詳しく** → Call 1 + Call 2（詳細プロファイル）を実施 → Step 3（**全 8 部署を scaffold**）

**はじめてモードの有効化**：`office/CLAUDE.md` の冒頭付近に `> 運用モード: はじめて（強めに誘導）` の 1 行を必ず書く。運営モードはこの行を見て [はじめてモードの挙動](#はじめてモードの挙動強めに誘導) を適用する。

#### Call 1Q: はじめてモードの最小ヒアリング（1 問、はじめてモードのみ）
...
Q2〜Q4 は「未定」扱い（後から `/caw` で補完）。**部署はモード共通で全 8 部署を作成する**。`office/CLAUDE.md` には研究分野のみ反映し、他は `{{未設定}}` プレースホルダで「`/caw` で後から設定を足せます」と注記。

#### Call 1: 研究プロファイル（4 問、通常 / 詳しく）
...
#### Call 2: 詳細プロファイル（4 問、詳しく のみ）

**詳しく**モードでのみ実施。回答は各部署 CLAUDE.md のパーソナライズメモに反映し、運用初期から精度を上げる。
...
**詳しく**で得た回答は `office/CLAUDE.md` の「パーソナライズメモ」に箇条書きで保存し、各部署が文脈として参照できるようにする。

### Step 3: 自動スキャフォールド

ヒアリング結果に基づいて、以下を一括生成する。

**scaffold 範囲（全モード共通：化学者モードは常に全 8 部署）**：
- **はじめて** → `office/` + ルート CLAUDE.md + **全 8 部署**（secretary / research / engineering / computation / analysis / writing / review / presentation）+ 作業ディレクトリ。Q2〜Q4 は未取得なので該当プレースホルダは `{{未設定}}`。完了メッセージで「`/caw` で設定を後から足せます」と案内
- **通常** → `office/` + ルート CLAUDE.md + **全 8 部署** + 作業ディレクトリ（Q1〜Q4 を各部署 CLAUDE.md に反映）
- **詳しく** → 通常と同じ + Call 2（Q6〜Q9）の回答を `office/CLAUDE.md` のパーソナライズメモと各部署 CLAUDE.md に反映 + Q8 で申請書予定があれば writing 部署に申請書トラッカー雛形を追加
...
**重要**：`office/.mcp-setup.md` は **手順書**であり、API key そのものは絶対に書かない（環境変数経由で渡す手順のみ記載）。はじめてモードでは Q3/Q4 未取得のため、MCP セットアップは生成せず「`/caw` で後から生成できます」と案内するに留める。
...
- caw を十分に使うには Python・poppler などの外部ツールが要ります。
  `/caw-setup` を実行すると、不足しているものを検出し、確認のうえ順番にインストールします
```

scaffold 完了後、不足ツールがありそうなら（例: スライドや図を使う予定なのに Python パッケージが無い）、
完了メッセージの後に「環境を整えるなら `/caw-setup` を実行してください」と一言添える。
