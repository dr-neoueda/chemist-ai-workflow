...
- **就活** を選んだら → **`references/job-hunting-departments.md` を読み、その §A〜§E に従って**就活モードのオンボーディング・scaffold・START HERE・運営モードを実行する（以降の研究向け Call 1〜3 は使わない）。経験レベル（Call 0）は就活トラックでも共通で聞き、はじめてモードの判定に使う。
