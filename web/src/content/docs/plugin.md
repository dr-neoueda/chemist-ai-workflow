...
| カテゴリ | 内容 |
|---|---|
| **Skills**（8） | `/caw`（オンボーディング → 自動スキャフォールド → 運営モード）<br>`/caw-research`（関心テーマの論文検索 → `work/topics/` に HTML リスト化・タイトルがリンク）<br>`/caw-paper`（入手 PDF のメタデータ抽出 + 書誌付き要約 + ナレッジベース・クラウドストレージ自動登録）<br>`/caw-input`（6 ソフトの入力ファイル雛形生成 + ジョブ記録）<br>`/caw-playbook`（計算 log 解析 → Lessons Learned 自動追記）<br>`/caw-doctor`（`office/` 構造の健全性チェック + 修復提示）<br>`/caw-setup`（前提ツール検出 → 計画提示 → 順次インストール）<br>`/caw-slides`（発表用 PowerPoint 生成・4 用途バリアント + スタイルガイド） |
